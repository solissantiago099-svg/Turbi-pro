...
export default function TaskNotifications({ tasks, user, driverId, onOpen }) {
  ...
  return (
    <div className="taskNotifications">
      <button type="button" className="iconBtn" aria-label={`Notificaciones: ${unread.length} sin leer`} aria-expanded={open} onClick={() => setOpen(!open)}>
        <Bell size={20} />
        {unread.length > 0 ? <span className="notificationBadge">{unread.length}</span> : null}
      </button>
      {open ? (
        <section className="notificationPanel" aria-label="Notificaciones de tareas">
          <header>
            <b>Notificaciones ({unread.length})</b>
            <button className="iconBtn" aria-label="Cerrar notificaciones" onClick={() => setOpen(false)}><X size={18} /></button>
          </header>
          {unread.length > 0 ? <button className="btn" onClick={() => markRead(items)}>Marcar todas como leidas</button> : null}
          {!items.length ? <p>No hay notificaciones de tareas.</p> : null}
          <div className="notificationList">
            {items.map((task) => (
              <button
                key={key(task)}
                className={`notificationItem ${read.includes(key(task)) ? "" : "unread"}`}
                onClick={() => {
                  markRead([task]);
                  setOpen(false);
                  onOpen(task);
